...
@author BinhoTM Fabriciojuniorsilva@gmail.com
"""
...
def main():
    ...
    # json.dump() will handle quote escaping automatically, so we only need to:
    # 1. Convert newlines to \r\n (this is what Pterodactyl expects)
    # Note: We DON'T manually escape quotes - json.dump() does that for us
    ...
    print("✅ Script synchronized successfully into egg JSON!")
    print(f"   📄 Size of script: {len(script_content)} bytes")
    print(f"   📝 Lines: {script_content.count(chr(10)) + 1}")
    print(f"   🔧 File: egg-pterodactyl-arma-reforger.json")
